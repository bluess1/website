...
function getRotationForNumber(numIndex) {
  // Pointer at top = 0 degrees (CSS 0deg)
  // Wheel starts with WHEEL_ORDER[0]=0 at 0deg
  // To land numIndex at top: need to rotate so it reaches 0deg
  return -(numIndex * POCKET_ANGLE);
}

const ROULETTE_BETS = [
  { id: 'red',   label: '🔴 RED',    cls: 'red-btn',   mult: 2,  payout: 'Pays 1:1', check: n => n > 0 && RED_NUMS.has(n) },
  { id: 'black', label: '⚫ BLACK',  cls: 'black-btn', mult: 2,  payout: 'Pays 1:1', check: n => n > 0 && !RED_NUMS.has(n) },
  { id: 'zero',  label: '🟢 ZERO',   cls: 'green-btn', mult: 35, payout: 'Pays 35:1', check: n => n === 0 },
  { id: 'double', label: '🟢 00',     cls: 'green-btn', mult: 35, payout: 'Pays 35:1', check: n => n === '00' },
  ...
  { id: '1st12', label: '1ST 12',     cls: '',          mult: 3,  payout: 'Pays 2:1', check: n => n >= 1 && n <= 12 },
  { id: '2nd12', label: '2ND 12',     cls: '',          mult: 3,  payout: 'Pays 2:1', check: n => n >= 13 && n <= 24 },
  { id: '3rd12', label: '3RD 12',     cls: '',          mult: 3,  payout: 'Pays 2:1', check: n => n >= 25 && n <= 36 },
];
    ...
    <div class="rl-layout">
      <div class="rl-left">
        <div class="rl-wheel-section">
          ...
          <div id="rl-live-feed" class="rl-live-feed">Waiting for spins...</div>
        </div>
        <div class="rl-bet-section">
          <div class="section-label">YOUR BET</div>
          <div class="bet-row">
      ...
      <div class="rl-right">
        ...
        <div class="rl-leaderboard-section">
          <div class="section-label">🏆 TOP PLAYERS</div>
          <div id="rl-leaderboard" class="rl-leaderboard"></div>
        </div>
      </div>
    </div>
  `;
...
function renderRlTable() {
  ...
  const betRows = [
    ...
    [null, '28', '29', '30', null],
    [null, '31', '32', '33', null],
    [null, '34', '35', '36', 'double'],
    ['low', 'odd', 'even', 'high', null],
  ];
  
  table.innerHTML = betRows.map(row => `
    <div class="rl-table-row">
      ${row.map((cell, i) => {
        if (!cell) return '<div class="rl-table-cell empty"></div>';
        if (cell === 'zero') return `<div class="rl-table-cell green-cell" onclick="rlSelect('zero')">0</div>`;
        if (cell === 'double') return `<div class="rl-table-cell green-cell" onclick="rlSelect('double')">00</div>`;
        if (cell === 'low') return `<div class="rl-table-cell" onclick="rlSelect('low')">1-18</div>`;
        if (cell === 'odd') return `<div class="rl-table-cell" onclick="rlSelect('odd')">ODD</div>`;
        if (cell === 'even') return `<div class="rl-table-cell" onclick="rlSelect('even')">EVEN</div>`;
        if (cell === 'high') return `<div class="rl-table-cell" onclick="rlSelect('high')">19-36</div>`;
        if (cell === '1st12') return `<div class="rl-table-cell" onclick="rlSelect('1st12')">1ST 12</div>`;
        if (cell === '2nd12') return `<div class="rl-table-cell" onclick="rlSelect('2nd12')">2ND 12</div>`;
        if (cell === '3rd12') return `<div class="rl-table-cell" onclick="rlSelect('3rd12')">3RD 12</div>`;
        const n = parseInt(cell);
        const isRed = RED_NUMS.has(n);
        const isGreen = GREEN_NUMS.has(n);
        let cls = '';
        if (isRed) cls = 'red-cell';
        else if (isGreen) cls = 'green-cell';
        else cls = 'black-cell';
        cls += rouletteSelected === 'red' && isRed ? ' selected' : '';
        cls += rouletteSelected === 'black' && !isRed && !isGreen ? ' selected' : '';
        return `<div class="rl-table-cell ${cls}" onclick="rlSelect('red')">${cell}</div>`;
      }).join('')}
    </div>
  `).join('');
}
...
function rlSelect(id) {
  rouletteSelected = id;
  const btns = document.querySelectorAll('.roulette-bet-btn, .rl-table-cell');
  btns.forEach(b => {
    b.classList.remove('selected');
    if (b.onclick && b.onclick.toString().includes(`'${id}'`)) {
...
async function loadRlLeaderboard() {
  ...
  container.innerHTML = data.slice(0, 5).map((p, i) => `
    <div class="rl-lb-entry">
      <span>${medals[i] || (i+1)} ${p.name}</span>
      <span class="rl-lb-balance">${State.fmt(p.balance)}</span>
    </div>
  `).join('');
}
...
function addRlPayout(num, won, amount) {
  ...
  if (rlRecentPayouts.length > 8) rlRecentPayouts.pop();
  updateRlPayoutFeed();
}
